...
(async () => {
	try {
		await replace({
			...
			from: /github.com\/ptkdev-boilerplate\/node-discord-bot-boilerplate/g,
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /github.com\/ptkdev\/node-discord-bot-boilerplate/g,
			to: setup.github_full_repository_url,
		});
	} catch (error) {
		console.error("Error occurred:", error);
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /@ptkdev\/node-discord-bot-boilerplate/g,
	...
	try {
		await replace({
			...
			from: /"projectName": "ptkdev-boilerplate\/node-discord-bot-boilerplate"/g,
